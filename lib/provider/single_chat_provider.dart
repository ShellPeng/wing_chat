...
class SingleChatProvider with ChangeNotifier {
  ...
  initConversation() {
    jmSingle = JMSingle.fromJson({
      ...
      "appKey": '65e831cf441e2df70442995a'
    });
  ...
  _createGroupConversation() async {
    jSingleConver = await JMessage.createConversation(target: jmSingle);

    JMessage.resetUnreadMessageCount(target: jmSingle);

    await JMessage.enterConversation(target: jmSingle);
  ...
  void dispose() {
    super.dispose();
    controller.dispose();
    JMessage.exitConversation(target: jmSingle);
